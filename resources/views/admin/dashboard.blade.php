...
<div class="space-y-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div class="bg-white/80 backdrop-blur-lg border border-slate-200 shadow-lg rounded-3xl p-6 relative overflow-hidden transition-all duration-300 hover:-translate-y-1">
            ...
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-xs font-semibold text-slate-500 uppercase tracking-widest">Total Pengguna Terdaftar</p>
                    <h3 class="text-3xl font-extrabold mt-2 tracking-tight text-[#1a365d]">{{ $totalUsers }}</h3>
                    <p class="text-xs text-slate-400 mt-1">Masyarakat & Admin aktif</p>
                </div>
        ...
        <div class="bg-white/80 backdrop-blur-lg border border-slate-200 shadow-lg rounded-3xl p-6 relative overflow-hidden transition-all duration-300 hover:-translate-y-1">
            ...
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-xs font-semibold text-slate-500 uppercase tracking-widest">Total Log Perjalanan</p>
                    <h3 class="text-3xl font-extrabold mt-2 tracking-tight text-[#1a365d]">{{ $totalLogs }}</h3>
                    <p class="text-xs text-slate-400 mt-1">Semua riwayat terdata</p>
                </div>
        ...
        <div class="bg-white/80 backdrop-blur-lg border border-slate-200 shadow-lg rounded-3xl p-6 relative overflow-hidden transition-all duration-300 hover:-translate-y-1">
            ...
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-xs font-semibold text-slate-500 uppercase tracking-widest">Suhu Tubuh Tinggi (Alert)</p>
                    ...
                    <p class="text-xs text-slate-400 mt-1">Suhu &ge; 37.5°C terdeteksi</p>
                </div>
    ...
    <div class="bg-white/80 backdrop-blur-lg border border-slate-200 shadow-sm rounded-3xl p-6 transition-all duration-300">
        <form action="{{ route('admin.dashboard') }}" method="GET" class="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div class="relative">
                <label class="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2">Cari Data</label>
                <div class="relative">
                    <span class="absolute inset-y-0 left-0 flex items-center pl-3 text-slate-400">
                        <i class="fa-solid fa-magnifying-glass"></i>
                    </span>
                    <input type="text" name="search" value="{{ $search }}"
                        class="w-full bg-white border border-slate-200 rounded-2xl pl-9 pr-4 py-2 text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-[#7da8b6] focus:border-transparent transition-all shadow-sm"
            ...
            <!-- Date Filter -->
            <div>
                <label class="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2">Filter Tanggal</label>
                <input type="date" name="filter_tanggal" value="{{ $filterTanggal }}"
                    class="w-full bg-white border border-slate-200 rounded-2xl px-4 py-2 text-slate-800 focus:outline-none focus:ring-2 focus:ring-[#7da8b6] focus:border-transparent transition-all shadow-sm">
            </div>
            
            <!-- Temperature Filter -->
            <div>
                <label class="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2">Kondisi Suhu</label>
            ...
            <div class="flex items-end space-x-2">
                <button type="submit" class="flex-1 bg-[#4a7a8a] hover:bg-[#3b6370] text-white font-semibold py-2.5 rounded-2xl transition-all duration-300 hover:-translate-y-0.5 cursor-pointer flex items-center justify-center space-x-2 shadow-sm">
                    <i class="fa-solid fa-filter"></i>
                    <span>Terapkan</span>
                </button>
                
                @if($search || $filterTanggal || $filterSuhu)
                    <a href="{{ route('admin.dashboard') }}" class="px-4 bg-rose-50 hover:bg-rose-100 border border-rose-200 text-rose-600 py-2 rounded-2xl transition-all duration-300 hover:-translate-y-0.5 flex items-center justify-center shadow-sm" title="Reset Pencarian">
    ...
    <div class="bg-white/80 backdrop-blur-lg border border-slate-200 shadow-xl rounded-3xl overflow-hidden transition-all duration-300" x-data="{ openDeleteModal: false, deleteUrl: '' }">
        ...
        <div class="px-6 py-4 bg-slate-50 border-b border-slate-200 flex items-center justify-between">
            ...
            <div class="flex items-center space-x-3">
                ...
                <span class="text-xs text-slate-500">Ditemukan: <strong class="text-slate-800">{{ $perjalanans->count() }}</strong> data log</span>
            </div>
        </div>
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-white border-b border-slate-200">
                        ...
                        <th class="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Lokasi Kunjungan</th>
                        <th class="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Suhu Tubuh</th>
                        <th class="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Catatan</th>
                        <th class="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider text-center">Tindakan</th>
                    </tr>
                </thead>
                        ...
                        <tr>
                            <td colspan="7" class="px-6 py-12 text-center text-slate-500">
                                <div class="flex flex-col items-center justify-center space-y-3">
                                    ...
                                    <p class="font-medium text-base text-slate-600">Tidak ada log perjalanan global</p>
                                    <p class="text-xs text-slate-400">Database bersih dari catatan perjalanan masyarakat.</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
             ...
             <div class="w-full max-w-md bg-white border border-slate-200 rounded-3xl p-6 shadow-2xl transform transition-all duration-300"
                  ...
                  <div class="flex items-start space-x-4">
                      ...
                      <div class="flex-1 mt-1">
                          <h3 class="text-lg font-bold text-slate-800">Konfirmasi Hapus Admin</h3>
                          <p class="text-sm text-slate-500 mt-1 body-font font-medium">Apakah Anda yakin ingin menghapus data log anomalus ini sebagai Admin? Tindakan ini akan menghapus log perjalanan dari pengguna tersebut secara permanen.</p>
                      </div>
                  </div>
