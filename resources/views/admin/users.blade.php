...
@section('page_header', __('Manajemen Pengguna Terdaftar'))
...
<div class="space-y-6">
    ...
    <div class="bg-white/80 backdrop-blur-lg border border-slate-200 shadow-sm rounded-3xl p-6 transition-all duration-300">
        <div class="flex items-center space-x-3">
            ...
            <div>
                <h2 class="text-xl font-bold text-[#1a365d]">{{ __('Manajemen Pengguna') }}</h2>
                <p class="text-xs text-slate-500">{{ __('Kelola akun masyarakat, tinjau aktivitas pengumpulan data, dan lakukan pembersihan data bermasalah.') }}</p>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white/80 backdrop-blur-lg border border-slate-200 shadow-xl rounded-3xl overflow-hidden transition-all duration-300" x-data="{ openDeleteModal: false, deleteUrl: '' }">
        ...
        <div class="px-6 py-4 bg-slate-50 border-b border-slate-200 flex items-center justify-between">
            <h3 class="font-bold text-base text-[#1a365d] flex items-center space-x-2">
                ...
                <span>{{ __('Tabel Anggota & Hak Akses') }}</span>
            </h3>
            <span class="text-xs text-slate-500">{{ __('Total Pengguna') }}: <strong class="text-slate-800">{{ $users->count() }}</strong></span>
        </div>
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-white border-b border-slate-200">
                        ...
                        <th class="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">{{ __('Nama Lengkap') }}</th>
                        <th class="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">{{ __('Alamat Email') }}</th>
                        <th class="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">{{ __('Tanggal Bergabung') }}</th>
                        <th class="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">{{ __('Total Catatan Log') }}</th>
                    ...
                    @forelse($users as $index => $userItem)
                        <tr class="hover:bg-slate-50 transition-all duration-200">
                            ...
                            <td class="px-6 py-4 text-sm font-semibold">
                                <div class="flex items-center space-x-3">
                                    @if($userItem->avatar)
                                        <img src="{{ asset('storage/' . $userItem->avatar) }}" alt="{{ $userItem->name }}" class="w-8 h-8 rounded-full object-cover border border-slate-200">
                                    @else
                                    ...
                                    <div class="flex flex-col">
                                        ...
                                        <span class="text-[10px] text-slate-500 mt-0.5 capitalize">
                                            @if($userItem->role === 'admin')
                                                <span class="text-[#4a7a8a] font-bold bg-[#5c8d9d]/10 px-1.5 py-0.5 rounded">{{ __('Admin') }}</span>
                                            @else
                                                <span class="text-slate-600 bg-slate-100 px-1.5 py-0.5 rounded">{{ __('Masyarakat') }}</span>
                                            @endif
                                        </span>
                                    </div>
                                </div>
                            ...
                            <td class="px-6 py-4 text-sm text-center">
                                @if($userItem->id !== Auth::id())
                                    <button type="button" 
                                            ...
                                            title="{{ __('Hapus Akun Pengguna') }}">
                                        <i class="fa-solid fa-user-minus mr-1"></i>{{ __('Hapus') }}
                                    </button>
                                @else
                                    <span class="text-xs text-slate-400 italic">{{ __('Akun Anda') }}</span>
                                @endif
                            </td>
                        </tr>
                    @empty
             ...
             <div class="w-full max-w-md bg-white border border-slate-200 rounded-3xl p-6 shadow-2xl transform transition-all duration-300"
                  ...
                  <div class="flex items-start space-x-4">
                      ...
                      <div class="flex-1 mt-1">
                          <h3 class="text-lg font-bold text-slate-800">{{ __('Hapus Akun Pengguna') }}</h3>
                          <p class="text-sm text-slate-500 mt-1 body-font">{{ __('Apakah Anda yakin ingin menghapus data ini?') }}</p>
                      </div>
                  </div>
